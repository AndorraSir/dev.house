    ...
    foreach($obj_widgets->SECONDARY as $key=>$obj)
    {
        ...
        if(!empty($title))
        {
            ...
            else
            {
                ...
                elseif(file_exists(FCPATH.'templates/'.$template_name.'/form_fields/secondary/'.$obj->type.'.php'))
                {
                    echo $CI->load->view($template_name.'/form_fields/secondary/'.$obj->type.'.php', array_merge($CI->data, array('field'=>$obj, 'field_data'=>$CI->fields[$obj->id])), true);
                }
                else
                {
                    echo 'MISSING TEMPLATE: '.$obj->type.'<br />';
	...
	if (count($array)) {
		...
		foreach ($array as $key=>$item) {
            ...
            if($position == 1 && $child == FALSE){
                $item['navigation_title'] = 'HOME<img src="assets/img/home-icon.png" alt="'.$item['navigation_title'].'" />';
...
function get_lang_menu ($array, $lang_code, $extra_ul_attributes = '')
{
    ...
    foreach ($array as $item) {
        ...
        if($CI->uri->segment(1) == $listing_uri)
        {
            if($active)
            {
                $str.='<li class="'.$item['code'].' active">'.anchor(slug_url($listing_uri.'/'.$CI->uri->segment(2).'/'.$item['code'].'/'.$CI->uri->segment(4)), $item['code'].$flag_icon).'</li>';
            }
            else
            {
                $property_title = '';
                ...
                $str.='<li class="'.$item['code'].'" >'.anchor(slug_url($listing_uri.'/'.$CI->uri->segment(2).'/'.$item['code'].'/'.url_title_cro($property_title)), $item['code'].$flag_icon).'</li>';
            }
        }
...
function cache_file($new_file, $original_file = NULL, $compress_enabled=true)
{
    static $file_list;
    $CI =& get_instance();
    $template_name = $CI->data['settings_template'];
    ...
    if($original_file === NULL)
    {
        ...
        if(filemtime($cache_file_abs) > time()-$cache_time_sec)
        {
            if(strpos($new_file, 'css') !== FALSE)
            {
                echo '<link rel="stylesheet" type="text/css" href="assets/cache/'.$new_file.'" media="screen">';
            }
