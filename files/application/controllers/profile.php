...
class Profile extends Frontend_Controller
{
    ...
    public function index()
    {
        ...
        if(count($agent))
        {
            ...
            $this->data['page_body']  = $agent['description'];
        ...
        if(!empty($agent['repository_id']))
            $where_in[] = $agent['repository_id'];
        ...
        // Thumbnail agent
        if(count($agent) && !empty($agent['image_user_filename']) && file_exists(FCPATH.'files/thumbnail/'.$agent['image_user_filename']))
        {
